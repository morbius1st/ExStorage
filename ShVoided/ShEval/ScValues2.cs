...
namespace ShEval
{
	...
	public class ScValues2
	{

		public Dictionary<KEY, Dictionary<ScFieldColumns, string>> ScFieldValues { get; set; }
		public Dictionary<KEY, Dictionary<ScFieldColumns, string>> ScDataValues { get; set; }
		

		public void setFieldsValues2(Dictionary<KEY, ScFieldDefMeta2> fieldList)
		{
			ScFieldValues = new Dictionary<KEY, Dictionary<ScFieldColumns, string>>();

			foreach (KeyValuePair<KEY, ScFieldDefMeta2> kvp in fieldList)
			{
				ScFieldDefMeta2 lf = kvp.Value;
		...
		public void setDataValues2<Tdata>(Dictionary<KEY, Tdata> fieldList)
			where Tdata :  IShScFieldData2, new()
		{
			ScDataValues = new Dictionary<KEY, Dictionary<ScFieldColumns, string>>();

			foreach (KeyValuePair<KEY, Tdata> kvp in fieldList)
			{
				IShScFieldData2 lf = kvp.Value;

				IShScFieldMeta2 fld = lf.Meta1Field;
