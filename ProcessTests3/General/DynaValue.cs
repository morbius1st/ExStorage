...
namespace UtilityLibrary
{
	public enum ChgSrcId
	{
		...
		CI_SRC_A, // regular field

		CI_SRC_D, // fam and type list
		CI_SRC_E, // debug items - lastid & sheets list
		CI_SRC_T, // name or date modified (by system)
		CI_SRC_X, // name modified (by user)
	}
	...
	#region revit specific ops
		...
		public bool ChangeValue(dynamic value, ChgSrcId cs)
		{
			...
			R.AddRouteEnter($"changing value [ {value} ] | chg src {cs} | track changes {TrackChanges}");
			...
			if (TrackChanges)
			{
				// updates is changed / is dirty
				ChangeQty = 1;
			}

			R.AddRoute($"is dirty {IsDirty}");
		...
		/// </summary>
		public void FixValue(bool isDirty, ChgSrcId cs)
		{
			IsChanged = isDirty;
