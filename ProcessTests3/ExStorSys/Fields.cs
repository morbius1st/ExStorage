...
namespace ExStorSys
{
	...
	public class Fields
	{
		...
		public static Dictionary<WorkBookFieldKeys, FieldDef<WorkBookFieldKeys>> WorkBookFields {get;} = new ()
		{
			...
			{PK_AD_LAST_ID,          new (PK_AD_LAST_ID,           "LastId",           "Last DS Identification Code" , nameof(WorkBook.LastId)        , new DynaValue("AAAA")                     , IU_SND_ALT_SRC_A  , FEL_DEBUG			, FC_TYPE_4   , [CI_SRC_A, CI_SRC_E])} ,
			{PK_AD_NAME_CREATED,     new (PK_AD_NAME_CREATED,      "CreateName",       "Creator's Name"              , nameof(WorkBook.NameCreated)   , new DynaValue(K_NOT_DEFINED_STR)          , IU_SND_RECORD     , FEL_ADV_VIEW_ONLY	, FC_TYPE_14  , [CI_NOP  , CI_NOP  ])} ,
			{PK_AD_DATE_CREATED,     new (PK_AD_DATE_CREATED,      "CreateDate",       "Date Created"                , nameof(WorkBook.DateCreated)   , new DynaValue(DateTime.Now.ToString("s")) , IU_SND_RECORD     , FEL_VIEW_ONLY		, FC_TYPE_14  , [CI_NOP  , CI_NOP  ])} ,
			{PK_AD_NAME_MODIFIED,    new (PK_AD_NAME_MODIFIED,     "ModifyName",       "Modifier's Name"             , nameof(WorkBook.NameModified)  , new DynaValue(K_NOT_DEFINED_STR)          , IU_SND_RECORD     , FEL_ADV_VIEW_ONLY	, FC_ALWAYS   , [CI_SRC_X, CI_SRC_T])} ,
			{PK_AD_DATE_MODIFIED,    new (PK_AD_DATE_MODIFIED,     "ModifyDate",       "Date Modified"               , nameof(WorkBook.DateModified)  , new DynaValue(K_NOT_DEFINED_STR)          , IU_SND_RECORD     , FEL_VIEW_ONLY		, FC_ALWAYS   , [CI_NOP  , CI_SRC_T])} ,
		...
		public static Dictionary<SheetFieldKeys, FieldDef<SheetFieldKeys>> SheetFields {get; } = new ()
		{
			...
			{RK_AD_NAME_CREATED,      new (RK_AD_NAME_CREATED,      "CreateName",       "Creator's Name"              , nameof(Sheet.NameCreated)    , new DynaValue(K_NOT_DEFINED_STR)       , IU_SND_RECORD    , FEL_ADV_VIEW_ONLY   , FC_TYPE_14  , [CI_NOP   , CI_NOP   ])} ,
 			{RK_AD_DATE_CREATED,      new (RK_AD_DATE_CREATED,      "CreateDate",       "Date Created"                , nameof(Sheet.DateCreated)    , new DynaValue(K_NOT_DEFINED_STR)       , IU_SND_RECORD    , FEL_VIEW_ONLY       , FC_TYPE_14  , [CI_NOP   , CI_NOP   ])} ,
			{RK_AD_NAME_MODIFIED,     new (RK_AD_NAME_MODIFIED,     "ModifyName",       "Modifier's Name"             , nameof(Sheet.NameModified)   , new DynaValue(K_NOT_DEFINED_STR)       , IU_SND_RECORD    , FEL_ADV_VIEW_ONLY   , FC_ALWAYS   , [CI_SRC_X , CI_SRC_T ])} ,
			{RK_AD_DATE_MODIFIED,     new (RK_AD_DATE_MODIFIED,     "ModifyDate",       "Date Modified"               , nameof(Sheet.DateModified)   , new DynaValue(K_NOT_DEFINED_STR)       , IU_SND_RECORD    , FEL_VIEW_ONLY       , FC_ALWAYS   , [CI_NOP   , CI_SRC_T ])} ,
