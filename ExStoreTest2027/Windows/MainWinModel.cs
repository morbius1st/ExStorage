...
namespace ExStoreTest2027.Windows
{
	...
	#region ctor
		...
		private void init()
		{
			...
			// workbook
			CmdWorkbookUndoChanges = new RelayCommandEx(cmdWorkbookUndoChgsExe, cmdWorkbookUndoChgsCanExe);
			CmdWorkbookApplyChanges = new RelayCommandEx(cmdWorkbookApplyChgsExe, cmdWorkbookApplyChgsCanExe);
			CmdWorkbookFieldUndo = new RelayCommand(cmdWorkbookFieldUndo, cmdWorkbookFieldUndoCanExe);

			// sheet
			CmdSheetUndoChanges = new RelayCommandEx(cmdSheetUndoChgsExe, cmdSheetUndoChgsCanExe);
			CmdSheetApplyChanges = new RelayCommandEx(cmdSheetApplyChgsExe, cmdSheetApplyChgsCanExe);
			CmdSheetFieldUndo = new RelayCommand(cmdSheetFieldUndo, cmdSheetFieldUndoCanExe);

			// family list
			CmdFamListUndoChanges = new RelayCommandEx(CmdFamListUndoChgsExe, CmdFamListUndoChgsCanExe);
			CmdSaveFamList = new RelayCommandEx(CmdSaveFamListExe, CmdSaveFamListCanExe);

			SaveNewFamilyListItem = new RelayCommandEx(SaveNewFamItemExe, SaveNewFamItemCanExe);
			ClearNewFamilyListItem = new RelayCommandEx(ClearNewFamItemExe, ClearNewFamItemCanExe);
			DeleteAFamilyListItem = new RelayCommandAlwaysCanExecute(deleteAFamItemExe);

			// sheet list
			CmdSheetAdd = new RelayCommandEx(cmdAddSheetExe, cmdAddSheetCanExe);
			CmdSheetDelete = new RelayCommandEx(cmdDeleteSheetExe, cmdDeleteSheetCanExe);
			CmdSheetUndo = new RelayCommandEx(cmdUndoSheetExe, cmdUndoSheetCanExe);
			CmdSheetListClear = new RelayCommandEx(cmdClearSheetListExe, cmdClearSheetListCanExe);
			CmdSheetListReset = new RelayCommandEx(cmdResetSheetListExe, cmdResetSheetListCanExe);
			CmdSheetListUndoChanges = new RelayCommandEx(cmdSheetListUndoChgsExe, cmdSheetListUndoChgsCanExe);
			CmdSheetListApplyChanges = new RelayCommandEx(cmdSheetListApplyChgsExe, cmdSheetListApplyChgsCanExe);
	...
	#region Ui Io
		...
		public KeyValuePair<string, FamAndType>? SelFamilyTypeItem
		{
			...
			set
			{
				...
				SaveNewFamilyListItem.RaiseCanExecuteChanged();
			}
		}
	...
	#region sheet list commands
		...
		/* ui index
		 * sheet
		 *	Undo Button:	CmdWorkbookUndoChanges => Wbk.UndoChangesAll()
		 *	Apply Button:	CmdWorkbookApplyChanges => xData.WorkbookApplyChgs(true)
		 */
		...
		public RelayCommandEx CmdSheetDelete { get; private set; }

		private void cmdDeleteSheetExe(object? parameter)
		{
			xData.DeleteSheet((string) parameter!);
		}
		...
		public RelayCommandEx CmdSheetUndo { get; private set; }

		private void cmdUndoSheetExe(object? parameter)
		{
			xData.UnDeleteSheet((string) parameter!);
		}
		...
		public RelayCommandEx CmdSheetAdd { get; private set; }
		...
		public RelayCommandEx CmdSheetListUndoChanges { get; private set; }
		...
		public RelayCommandEx CmdSheetListApplyChanges { get; private set; }
		...
		public RelayCommandEx CmdSheetListClear { get; private set; }
		...
		public RelayCommandEx CmdSheetListReset { get; private set; }
	...
	#region workbook commands
		...
		public RelayCommandEx CmdWorkbookUndoChanges { get; private set; }
		...
		public RelayCommandEx CmdWorkbookApplyChanges { get; private set; }
		...
		// field 
		public RelayCommand CmdWorkbookFieldUndo { get; private set; }

		private void cmdWorkbookFieldUndo(object? parameter)
		{
			if (parameter == null || 
				!(parameter is Tuple<object, object>)) return;

			// Debug.WriteLine($"+++> relay command EXE got | {parameter?.GetType().Name ?? "null"}");

			FieldData<WorkBookFieldKeys>? fd = (parameter as Tuple<object, object>)!.Item2 as FieldData<WorkBookFieldKeys>;
			
			if (fd == null) return;
			
			Wbk.UndoChange(fd, false);
		}

		private bool cmdWorkbookFieldUndoCanExe(object? parameter)
		{
			if (parameter == null || 
				!(parameter is Tuple<object, object>)) return false;

			// Debug.WriteLine($"+++> relay command CAN UNDO got | {parameter?.GetType().Name ?? "null"}");

			FieldData<WorkBookFieldKeys>? fd = (parameter as Tuple<object, object>)!.Item2 as FieldData<WorkBookFieldKeys>;
			
			// Debug.Write($"+++> wbk - undo can exe | is field data | {fd?.Field.FieldName ?? "null"} | can undo? {fd?.CanUndo.ToString() ?? "null"} ");
			// Debug.WriteLine($"| is changed? {fd?.DyValue.IsChanged.ToString() ?? "null"} | chg src {fd?.ChgSrc.ToString() ?? "null"}");
			
			return fd?.CanUndo ?? false;
		}
	...
	#region sheet commands
		...
		public RelayCommandEx CmdSheetUndoChanges { get; private set; }
		...
		private void cmdSheetUndoChgsExe(object? parameter)
		{
			...
			R.ProcessMsg($"SHT undo | sht modified? {CurrSht.IsModifiedExo}");

			CurrSht?.UndoChangesAll();

			R.ProcessMsg($"SHT undone | sht modified? {CurrSht.IsModifiedExo}", false);
		}
		...
		public RelayCommandEx CmdSheetApplyChanges { get; private set; }
		...
		// field 
		public RelayCommand CmdSheetFieldUndo { get; private set; }

		private void cmdSheetFieldUndo(object? parameter)
		{
			if (parameter == null ||
				!(parameter is Tuple<object, object>)) return;

			FieldData<SheetFieldKeys>? fd = (parameter as Tuple<object, object>)!.Item2 as FieldData<SheetFieldKeys>;

			if (fd == null) return;

			CurrSht!.UndoChange(fd, false);
		}

		private bool cmdSheetFieldUndoCanExe(object? parameter)
		{
			if (parameter == null ||
				!(parameter is Tuple<object, object>)) return false;
		...
		// // field 
		// public RelayCommandEx CmdSheetFieldUndo { get; private set; }
		//
		// private void cmdSheetFieldUndo(object? parameter)
		// {
		...
		// 	// if (btn == null) return;
		// 	//
		// 	FieldData<SheetFieldKeys>? fd = parameter as FieldData<SheetFieldKeys>;
		// 	
		// 	if (fd == null) return;
		//
		// 	CurrSht.UndoChange(fd, false);
	...
	#region family list commands
		...
		public RelayCommandEx CmdFamListUndoChanges { get; private set; }
		...
		public void CmdFamListUndoChgsExe(object? parameter)
		{
			...
			CurrSht?.UndoFamAndTypeListChanges();
		}
		...
		public RelayCommandEx CmdSaveFamList { get; private set; }

		public void CmdSaveFamListExe(object? parameter)
		{
			// CurrSht?.ApplyFamAndTypeChanges();
		...
		public bool CmdSaveFamListCanExe(object? parameter)
		{
			...
			return CurrSht.IsModifiedFamListWkg;
		...
		public RelayCommandEx SaveNewFamilyListItem { get; private set; }
		...
		public RelayCommandEx ClearNewFamilyListItem { get; private set; }
