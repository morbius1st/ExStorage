...
namespace ExStoreTest2027.Windows
{
	...
	public partial class MainWindow : INotifyPropertyChanged, IWin
	{
		...
		private void init()
		{
			...
			R.ProcessMsg($"got wbk {xData.WorkBook != null} | got shts {xData.GotAnySheets}", true , ObjectId);
			...
			R.ProcessMsg("pre-init comps", false, ObjectId);
		}
		...
		{
			R.ProcessMsg($"*** window loaded | model is {R.RvtDoc?.Title ?? "no title"} | model name | {ExStorMgr.Instance.xData.WorkBook?.ModelTitle ?? "null"}", null, -1);
		...
		private void statusReport(string title, int which = 1)
		{
			...
			if (which == 0)
			{
				...
				string shtLstModStat = xData.ApplyBtnShtsLstStatus  ? "is modified" : "not modified";
		...
		private void BtnUndoSht_OnClick(object sender, RoutedEventArgs e)
		{
			...
			Wm.CurrSht?.UndoChange(fd, false);
		}
		...
		private void BtnDebug_OnClick(object sender, RoutedEventArgs e)
		{
			...
			bool shtIsMod = shtCurr.IsModifiedExo;
			bool shtFatIsMod = shtCurr.IsModifiedFamListWkg;
			bool shtUndoBtn = shtCurr.UndoBtnStatus;
			bool shtApplyBtn = shtCurr.ApplyBtnStatus;

			int n1 = shtCurr.FamilyListCnt;
			int n2 = shtCurr.FamilyListWkgCnt;
			int n3 = shtCurr.FamilyListWkgCntNew;
			int n4 = shtCurr.FamilyListWkgCntDel;
			int n5 = shtCurr.FamilyListWkgCntMod;

			bool b1 = shtCurr.NameModifiedField.CanUndo;
			ChgSrcId cs1 = shtCurr.NameModifiedField.ChgSrc;

			bool b2 = wbk.NameModifiedField.CanUndo;
			ChgSrcId cs2 = wbk.NameModifiedField.ChgSrc;
