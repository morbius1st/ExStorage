...
namespace ExStorSys
{
	...
	public class Sheet : ExStorDataObj<SheetFieldKeys>
	{
		...
		private int familyListWkgCntMod;
		private int familyListWkgCntDel;
		private int familyListWkgCntNew;
	...
	#region shortcuts & properties
		...
		public override bool IsModifiedExo
		{
			...
			set
			{
				...
				OnPropChgd(new PropChgEvtArgs(PropertyId.PI_SHT_IS_MOD_EXO, value));
			}
		}

		/// <summary>
		/// is the apply button to be enabled
		/// </summary>
		public override bool ApplyBtnStatus
		{
			get => applyBtnStatus;
			set
			{
				...
				OnPropertyChanged();
				OnPropChgd(PropertyId.PI_SHT_APPLY_BTN_STAT, value);
			}
		}

		/// <summary>
		/// is the undo button to be enabled
		/// </summary>
		public override bool UndoBtnStatus
		{
			get => undoBtnStatus;
			set
			{
				...
				undoBtnStatus = value;
				OnPropertyChanged();
				OnPropChgd(PropertyId.PI_SHT_UNDO_BTN_STAT, value);
			}
		}
		...
		public bool IsModifiedFamListWkg
		{
			...
			set
			{
				...
				OnPropChgd(PropertyId.PI_SHT_IS_MOD_EXO, IsModifiedExo);
				OnPropChgd(PropertyId.PI_SHT_IS_MOD_FAM_LST_WKG, value);
			}
		}
	...
	#region general methods & creation
		...
		{
			R.ProcessMsg($"SHT enter | sht modified? {IsModifiedExo}", true, ObjectId);

			UndoFamAndTypeListChanges();

			base.UndoChangesAll();

			R.ProcessMsg($"SHT exit | sht modified? {IsModifiedExo}", false, ObjectId);
		}
	...
	#region limited editing
		...
		private Dictionary<string, string?> _FamilyList
		{
			...
			set
			{
				...
				OnPropertyChanged();
				OnPropChgd(PropertyId.PI_SHT_FAM_LST_IS_DIRTY, FamilyListField.IsDirty());
			}
		}
	...
	#region family names and types list
		...
		public int FamilyListWkgCntNew
		{
			get => familyListWkgCntNew;
			set
			{
				if (value == familyListWkgCntNew) return;
				familyListWkgCntNew = value;
				OnPropertyChanged();
			}
		}

		public int FamilyListWkgCntDel
		{
			get => familyListWkgCntDel;
			set
			{
				if (value == familyListWkgCntDel) return;
				familyListWkgCntDel = value;
				OnPropertyChanged();
			}
		}

		public int FamilyListWkgCntMod
		{
			get => familyListWkgCntMod;
			set
			{
				if (value == familyListWkgCntMod) return;
				familyListWkgCntMod = value;
				OnPropertyChanged();
			}
		}
		...
		// ReSharper disable once MemberCanBePrivate.Global
		/// <summary>
		/// Gets a value indicating whether the family list contains any elements
		/// </summary>
		public bool FamLstHasElements => FamilyListCnt > 0;

		/// <summary>
		/// determine if the family list as a key
		/// </summary>
		...
		/// <summary>
		/// determine if the family list WKG as a key
		/// </summary>
		...
		{
			R.ProcessMsg($"FAT undo enter | sht modified? {IsModifiedExo}");
			...
			R.ProcessMsg($"FAT undo exit | sht modified? {IsModifiedExo}", false);
		}
		...
		private void updateFamListProps()
		{
			...
			OnPropertyChanged(nameof(FamilyListWkgCnt));
		...
		{
			R.ProcessMsg($"Update Fam Lst enter | sht modified? {IsModifiedExo}");
			...
			R.ProcessMsg($"Update Fam Lst exit | sht modified? {IsModifiedExo}");
		}
		...
		private void updateFamLstCounts()
		{
			int famLstNew = 0;
			int famLstMod = 0;
			...
			foreach ((string key, FamAndType fat) in FamListWkg)
			{
				if (fat.IsNewItemFat) famLstNew++;
				if (fat.IsModifiedFat) famLstMod++;
			}

			FamilyListWkgCntDel = FamilyListCnt - FamilyListWkgCnt + famLstNew;
			FamilyListWkgCntNew = famLstNew;
			FamilyListWkgCntMod = famLstMod;
		}
