...
namespace ExStorSys
{
		...
		where TE : Enum
	{
		...
		// this is a property for the property - setup
		// this way for debugging only
		protected bool isModifiedExo
		{
			get => isModExo;
			set
			{
				// Debug.WriteLine($"** is mod exo FIELD set to {value}");
				isModExo = value;
			}
		}

		public abstract bool ApplyBtnStatus { get; set; }
		// {
		// 	get => applyBtnStatus;
		// 	set
		// 	{
		...
		// 		OnPropertyChanged();
		// 		OnPropChgd(PropertyId.PI_WBK_APPLY_BTN_STAT, value);
		// 	}
		// }

		public abstract bool UndoBtnStatus { get; set; }
		// {
		// 	get => undoBtnStatus;
		// 	set
		// 	{
		...
		// 		undoBtnStatus = value;
		// 		OnPropertyChanged();
		// 		OnPropChgd(PropertyId.PI_WBK_UNDO_BTN_STAT, value);
		// 	}
		// }
		...
		// [DebuggerStepThrough]
		protected void OnPropChgd(PropChgEvtArgs e)
		{
		...
		{
			OnPropChgd(new PropChgEvtArgs(pi, value));
		}
		...
		/// <summary>
		/// update the DS & E objects
		/// </summary>
		public bool UpdateExsObjects(DataStorage ds, Entity e)
		{
		...
		/// </summary>
		private void ModDate_Update()
		{
			SetDateModifiedByInternal(DateTime.Now.ToString("s"), ChgSrcId.CI_SRC_T);
		}

		/// <summary>
		/// undo the date modified - to be called from validate and UI<br/>
		/// </summary>
		private void ModDate_Undo()
		{
		...
		private void ModName_Update()
		{
			...
			SetNameModifiedInternal(ExStorConst.UserName, ChgSrcId.CI_SRC_T);
		}
		...
		{
			R.ProcessMsg($"validate enter | chg type {gotChgType}");
			...
			{
				R.ProcessMsg($"validate | got chg src A (or X)");
			...
			{
				R.ProcessMsg($"validate | got chg src D");
			...
			{
				R.ProcessMsg($"validate | got chg src T or X");

				// alt fix - using adjust A to include X
				// if (chgSrcs[(int) ChgSrcId.CI_SRC_X] > 0 && gotChgType == ChangeType.CT_CHANGE)
				// {
				// 	R.ProcessMsg($"got X and chg type is tc_change");
				// 	ModDate_Update();
				// 	ModName_Update();
				//
				// 	IsModifiedExo = true;
