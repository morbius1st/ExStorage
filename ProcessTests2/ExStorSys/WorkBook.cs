...
namespace ExStorSys
{
	...
	#region view only
		...
		public override void SetDateModifiedByInternal(string value, ChgSrcId cs)
		{
			...
			R.WriteLine($"\t\tSET DATE MODIFIED() | was name {DateModifiedField.DyValue!.Value} vs. new name {value}");
			R.WriteLine($"\t\tSET DATE MODIFIED() | prior is {DateModifiedField.DyValue.PriorValue ?? "null"}");

			SetNewValueDyn(DateModifiedField, value, cs);

			R.WriteLine($"\t\tSET DATE MODIFIED() | chgSrcId changed to [si_src] | is dirty? {DateModifiedField.IsDirty()}\n");
	...
	#region limited editing
		...
		public override string NameModified
		{
			...
			set
			{
				...
				R.WriteLine($"\tNAME MODIFIED_SET | chgSrcId is now [ {NameModifiedField.ChgSrcStd} ] | is dirty? {NameModifiedField.IsDirty()}");
		...
		public override void SetNameModifiedInternal(string value, ChgSrcId cs)
		{
			...
			R.WriteLine($"\t\tSET NAME MODIFIED() | prior is {NameModifiedField.DyValue.PriorValue ?? "null"}");
