...
// todo: either use a provided datastore or create a new datastore
		...
		public ExStoreRtnCode WriteField<TKey, TFld>(TKey keyName, DynaValue value, AShScFields<TKey, TFld> data, DataStorage ds, Entity e, Schema s)
			where TKey : Enum
			where TFld : IShScFieldData1<TKey>, new()
		{
			string s1;
			bool b1 = e.WriteAccessGranted();

			ExStoreRtnCode rtnCode = ExStoreRtnCode.XRC_GOOD;

			Field f = s.GetField(data.Fields[keyName].FieldName);

			if (f == null || !f.IsValidObject) return ExStoreRtnCode.XRC_FIELD_NOT_FOUND;
			...
			try
			{
				...
				rtnCode = updateEntity(ds, e);

				s1 = e.Get<string>(f);
			}
			catch 
			{
				return ExStoreRtnCode.XRC_FIELD_WRITE_FAIL;
			}

			return rtnCode;
		}

		private ExStoreRtnCode updateEntity(DataStorage ds, Entity e)
		{
			ExStoreRtnCode rtnCode = ExStoreRtnCode.XRC_GOOD;
			...
			using (Transaction T = new Transaction(smR.Exid.Document, "Save Cells Sheet Data"))
			{
				T.Start();
				{
					...
					{
						T.RollBack();
						rtnCode = ExStoreRtnCode.XRC_SCHEMA_WRITE_FAIL;
					}
				}
			}
...
// D1
			...
			where TFields : IShScFieldData1<TKey>, new()
		{
			...
			foreach (KeyValuePair<TKey, TFields> kvp in fields)
			{
				...
				if (kvp.Value.DyValue.IsDictStringString)
				{
					fb = sb.AddMapField(kvp.Value.FieldName, kvp.Value.DyValue.RevitGenericArg0TypeIs, kvp.Value.DyValue.RevitGenericArg1TypeIs);
				}
				else if (kvp.Value.DyValue.IsListString)
				{
					fb = sb.AddArrayField(kvp.Value.FieldName, kvp.Value.DyValue.RevitGenericArg0TypeIs);
				}
				else
				{
					fb = sb.AddSimpleField(kvp.Value.FieldName, kvp.Value.DyValue.RevitTypeIs);
				}
...
// WF
			...
			where TFld : IShScFieldData1<TKey>, new()
		{
			...
			{
				e.Set<IDictionary<string, string>>(f, field.DyValue.RevitValue);
			}
			else if (field.DyValue.IsListString)
			{
				e.Set<IList<string>>(f, field.DyValue.RevitValue);
			}
			else
			{
	...
	#region restore data
			...
			where TFlds :  IShScFieldData1<TKey>, new()
		{
			foreach (KeyValuePair<TKey, TFlds> kvp in data.Fields)
			{
				...
				{
					kvp.Value.SetValue = e.Get<IDictionary<string, string>>(kvp.Value.FieldName);
				}
				else if (t == typeof(List<string>))
				{
					kvp.Value.SetValue = e.Get<IList<string>>(kvp.Value.FieldName);
				}
		...
		public void ChangeSheet(Entity e, Field f, DynaValue value)
		{
			if (f.ValueType == typeof(string))
			{
				e.Set(f, value.AsString());
			}
